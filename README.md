...
````
The `scale` parameter controls the resizing of the original histology image. The height and width of the image will be adjusted to the percentage specified by the `scale` parameter.</br>
...
An anndata object storing the expression profiles and locations of cells will be saved.
...
To run with Visium data, you need to provide paths for the files below:  
