...
def result_stats(dataset, merged, startx, starty, patchsize):
    ...
    with open(dataset + '/results/cell_stats_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.txt', 'w') as fw:
...
def postprocess(dataset, startx, starty, patchsize, bin_size, dia_estimate, min_sz):
    ...
    intensity, dx, dy, pred_U, pred_V, pred_C = read_gradient(dataset + '/results/spot_prediction_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.txt', adata, (int(patchsizex), int(patchsizey)), bin_size, dia_estimate)
    ...
    fw = open(dataset + '/results/spot2cell_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.txt', 'w')
    for i in range(merged.shape[0]):
        for j in range(merged.shape[1]):
            if merged[i, j] > 0:
    ...
    plt.savefig(dataset + '/results/cell_masks_' + startx + ':' + starty + ':' + patchsize + ':' + patchsize + '.png')
