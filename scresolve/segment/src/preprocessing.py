...
def preprocess(dataset, bin_file, image_file, prealigned, align, startx, starty, patchsize, bin_size, ws_otsu_classes, ws_otsu_index, bg_th, n_neighbor):
    #read data
    if prealigned:
            ...
            if adatasub.layers['watershed_labels'][i, j] > 0:
                ...
                if idx >= 0 and idx < all_exp_merged_bins.shape[0] and np.sum(all_exp_merged_bins[idx, :]) > 0:
                    ...
                    for nucleus in watershed2center:
                        if (i - watershed2center[nucleus][0]) ** 2 + (j - watershed2center[nucleus][1]) ** 2 <= 900 or adatasub.layers['stain'][i, j] > bg_th:
                            backgroud = False
                            break
                    if backgroud:
