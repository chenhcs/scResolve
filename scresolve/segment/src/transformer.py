...
def run_experiment(dataset, startx, starty, patchsize, model, x_train, x_train_pos, x_train_, x_train_pos_, y_train, y_train_, y_binary_train, x_test, x_test_pos, x_validation, x_validation_pos, y_validation, y_binary_validation, learning_rate, weight_decay, batch_size, num_epochs):
    ...
    checkpoint_filepath = os.path.join('./' + dataset + 'ckpt', 'model_' + startx + '_' + starty + '_' + patchsize, 'ckpt')
...
def train(dataset, startx, starty, patchsize, epochs):
    startx = str(startx)
    starty = str(starty)
    ...
    pos_num = len(np.where(y_binary_train_ == 1)[0])
    neg_num = len(np.where(y_binary_train_ == 0)[0])
    print(pos_num, neg_num)
    if pos_num > neg_num:
        neg_index = np.array(np.where(y_binary_train_ == 0)[0])
        neg_index = np.repeat(neg_index, 50)
        np.random.shuffle(neg_index)
        neg_upsample = neg_index[:pos_num - neg_num]
        x_train_ = np.concatenate((x_train_, x_train_[neg_upsample]), axis=0)
        x_train_pos_ = np.concatenate((x_train_pos_, x_train_pos_[neg_upsample]), axis=0)
        y_train_ = np.concatenate((y_train_, y_train_[neg_upsample]), axis=0)
    ...
    for i in range(len(x_train_pos_)):
        #if x_train_pos_[i][0][0] > 2000 and x_train_pos_[i][0][1] > 2000:
    ...
    run_experiment(dataset, startx, starty, patchsize, transformer_classifier, x_train, x_train_pos, x_train_, x_train_pos_, y_train, y_train_, y_binary_train, x_test, x_test_pos, x_validation, x_validation_pos, y_validation, y_binary_validation, learning_rate, weight_decay, batch_size, num_epochs)
