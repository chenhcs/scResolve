...
from .super_resolution import super_resolution as _super_resolution
...
def _init(f):
    @click.option(
        "--save-path",
        type=click.Path(),
        default=f"scresolve-{dt.now().isoformat()}",
...
@click.group("convert")
def _convert():
    r"""Converts data of various formats to the format used by super-resolution model."""
...
def super_resolution(
    ...
    r"""
    Runs super-resolution model based on a project configuration file.
    ...
    with base_session:
        ...
        if expansion_strategy is None:
            expansion_strategy = expansion_strategies[
                config["super_resolution_expansion_strategy"]["type"]
            ](
                **config["super_resolution_expansion_strategy"][
                    config["super_resolution_expansion_strategy"]["type"]
                ]
            )
        ...
        analyses = {
            ...
            for name, settings in config["super_resolution_analyses"].items()
        }
        ...
        with Session(
            ...
            _super_resolution(
                ...
                network_depth=config["super_resolution_model"]["network_depth"],
                network_width=config["super_resolution_model"]["network_width"],
                min_counts=config["super_resolution_model"]["min_counts"],
                gene_regex=config["super_resolution_model"]["gene_regex"],
                patch_size=config["super_resolution_optimization"]["patch_size"],
                batch_size=config["super_resolution_optimization"]["batch_size"],
                epochs=config["super_resolution_optimization"]["epochs"],
                learning_rate=config["super_resolution_optimization"]["learning_rate"],
...
cli.add_command(super_resolution)
...
@click.option("--count", type=str, required=True)
